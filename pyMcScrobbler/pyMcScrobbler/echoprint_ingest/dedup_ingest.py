...
sys.path.insert(0, "/media/data/stephane/Documents/Code/McScrobbler/libs/echoprint-server/API")
import fp

_codegen_path = "/media/data/stephane/Documents/Code/McScrobbler/libs/echoprint-codegen/echoprint-codegen"
...
def collect_mp3s(list, dir):
    ...
    if os.path.isdir(dir):
        ...
        for f in files:
            ...
            elif fp.lower().endswith('.mp3') or fp.lower().endswith('.flac') or fp.lower().endswith('.m4a'):
                list.append(fp)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print "Usage: dedup_ingest.py --reset"
        print "Usage: dedup_ingest.py path-to-mp3s"
        sys.exit(1)
